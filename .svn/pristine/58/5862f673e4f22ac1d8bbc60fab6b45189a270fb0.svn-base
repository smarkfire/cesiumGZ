            ...
            getLocData: function (url, self, txt, maps, where) {
                $.ajax({
                    ...
                    success: function (result) {
                        $("#水文站").text("水文站("+result.features.length+")");
                        datas = [];
                ...
                $.ajax({
                    ...
                    success: function (result) {
                        $("#水位站").text("水位站("+result.features.length+")");
                        datas = [];
                        if (result.features.length > 0) {
                            for (var i = 0; i < result.features.length; i++) {
                                if (result.features[i].attributes.STCD!=0 && result.features[i].attributes.STCD!=null){
                ...
                $.ajax({
                    ...
                    success: function (result) {
                        $("#雨量站").text("雨量站("+result.features.length+")");
                        datas = [];
                        if (result.features.length > 0) {
                            for (var i = 0; i < result.features.length; i++) {
                                if (result.features[i].attributes.STCD!=0 && result.features[i].attributes.STCD!=null){
                        ...
                        self.pageColor(datas,"雨量站");
                    }
                });

            },
            //前端分页变色
            pageColor: function(datas,txt){
                var self = this;


                // 获取当前选中的文本
                var target = document.getElementsByClassName("loc_click on");
                var txt2 = target[0].innerText;

                if (txt2.indexOf(txt)!= -1 ){
                    //同时显示到layer
