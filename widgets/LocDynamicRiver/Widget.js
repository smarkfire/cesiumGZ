...
define([
	...
	function (declare,
		...
		return declare([BaseWidget], {
			baseClass: 'jimu-widget-LocDynamicRiver',
			name: 'LocDynamicRiver',
			riverWidth: 200,
			riverHeight: 10,
			speed: 10,
			positions: [],
			...
			startup: function () {
				topic.subscribe("ooooo", lang.hitch(this, this.oooo));
				...
				$('.close-dynamicriver').click(function () {
					$('.jimu-widget-LocDynamicRiver').hide();
				})
			...
			oooo: function (item) {
				if (item == name) {
					$('.jimu-widget-LocDynamicRive').stop().show();
				}
			},
			...
			prepareMaterial: function () {
				var e = new Cesium.Material({
					fabric: {
						uniforms: {
							image: "widgets/LocDynamicRiver/images/movingRiver.png",
							alpha: 0.5,
